...
(function() {
    ...
    var knownOptions = {
        checkboxes: {
            ...
            canReportSmokeDetectorOther:       new CheckboxOption(false, 'Other rooms: Show request types for Smoke Detector.', 'For target rooms other than SOCVR, show request type options for reporting to Smoke Detector (SD) that the question is spam/offensive, or that all the user\'s posts are spam/offensive. Using SD requires that you are approved to do so in that Room. If you\'re not yet approved, sending such reports will just have SD respond saying that you\'re not approved.'),
    ...
    function getRequestTypeByQuestionStatus(inPost) {
        ...
        if (onlySdSpamOffensive) {
            return ((isSOCVR && configOptions.checkboxes.canReportSmokeDetectorSOCVR) || (!isSOCVR && configOptions.checkboxes.canReportSmokeDetectorOther)) ? 'sd-report' : 'spam';
        } //else
