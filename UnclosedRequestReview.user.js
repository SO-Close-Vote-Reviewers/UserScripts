...
// @include      /^https?://chat\.stackoverflow\.com/search.*[?&]room=(?:41570|90230|126195|68414|111347)(?:\b.*$|$)/
// @include      /^https?://chat\.stackoverflow\.com/transcript/(?:41570|90230|126195|68414|111347)(?:\b.*$|$)/
    ...
    */
    ...
    funcs.addRequestStylesToDOM = () => {
        ...
        funcs.addStylesToDOM('urrsRequestStyles', [
            ...
            (config.nonUi.chatCompleteRequestsFade ? [
                ...
                '.monologue.urrsRequestComplete .message.urrsRequestComplete:not(:hover):not(.reply-parent):not(.reply-child) {',
                '    opacity: .4;',
                '}',
                '.monologue:not(.urrsRequestComplete) .message.urrsRequestComplete:not(:hover):not(.reply-parent):not(.reply-child) {',
                '    opacity: .4;',
                '}',
                '.monologue.urrsRequestComplete:hover .message.urrsRequestComplete:not(:hover):not(.reply-parent):not(.reply-child) {',
                '    opacity: .4;',
                '}',
    ...
    funcs.ui.createOptionsDialog = () => {
        ...
        dialog.innerHTML = [
            ...
            '                                In requests: Also, add 10k+ tags for (un)delete requests.',
            '                            </label>',
            ...
            '                            </label>',
            '                            <label title="You will need to reload the SOCVR chat room page for this to take effect.">',
            '                                <input type="checkbox" id="urrsOptionsCheckbox-chatUseVisitedLinkStyle"/>',
            '                                Blue visited links and show ♦ for moderators.',
            '                            </label>',
    ...
    if (isSearch && (isSearchCv || isSearchDel || isSearchReopen)) {
        ...
        let searchButton = funcs.ui.addButton('cv-', 'Open the cv-pls requests search page.', function(event) {
            GM.openInTab(window.location.origin + '/search?q=tagged%2Fcv&room=41570&page=1&pagesize=100&sort=newest');
            event.target.blur();
        });
        if (searchButton) {
        ...
        searchButton = funcs.ui.addButton('del-', 'Open the del-pls requests search page.', function(event) {
            ...
            GM.openInTab(window.location.origin + '/search?q=tagged%2Fdel+OR+tagged%2Fdelv+OR+tagged%2Fdelete+OR+tagged%2Fdv&user=&room=41570&page=1&pagesize=100&sort=newest');
            event.target.blur();
        });
        if (searchButton) {
        ...
        searchButton = funcs.ui.addButton('reopen-', 'Open the reopen-pls requests search page.', function(event) {
            ...
            GM.openInTab(window.location.origin + '/search?q=tagged%2Freopen&room=41570&page=1&pagesize=100&sort=newest');
            event.target.blur();
        });
        if (searchButton) {
        ...
        searchButton = funcs.ui.addButton('undel-', 'Open the undel-pls requests search page.', function(event) {
            //Search for 'reopen' tags:
            GM.openInTab(window.location.origin + '/search?q=tagged%2Fundel+OR+tagged%2Fundelete+OR+tagged%2Fundelv&room=41570&page=1&pagesize=100&sort=newest');
            event.target.blur();
        });
        if (searchButton) {
